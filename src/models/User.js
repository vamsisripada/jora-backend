...
const User = {
  ...
  async create({ name, email, phone, password, verificationToken, verificationExpires, role = "customer" }) {
    const db = getDb();
    ...
    const [result] = await db.execute(
      `
        INSERT INTO authUser
          (name, email, phone, password, role, tokenSalt, isVerified, verificationToken, verificationExpires)
        VALUES
          (?, ?, ?, ?, ?, ?, 0, ?, ?)
      `,
      [
  ...
  async updateRole(id, role) {
    ...
    await db.execute("UPDATE authUser SET role = ? WHERE id = ?", [role, id]);
  },
};
