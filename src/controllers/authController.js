...
const createAccessToken = (user) => {
  return jwt.sign(
    { id: user._id, email: user.email, tokenSalt: user.tokenSalt },
...
const createRefreshToken = (user) => {
  return jwt.sign(
    { id: user._id, tokenSalt: user.tokenSalt, type: "refresh" },
...
const setRefreshTokenOnUser = async (user, refreshToken) => {
  ...
  user.refreshTokenHash = await bcrypt.hash(refreshToken, refreshTokenSalt);
  user.refreshTokenExpires = new Date(Date.now() + 1000 * 60 * 60 * 24 * 30);
  await user.save();
};
...
export const me = async (req, res, next) => {
  try {
    const user = await User.findById(req.user.id).select(
      "_id name email phone isVerified"
    );
